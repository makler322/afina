...
void ServerImpl::OnRun() {
    ...
    while (running.load()) {
        ...
        {
            struct timeval tv;
            tv.tv_sec = 2; // TODO: make it configurable
            tv.tv_usec = 0;
...

        {
            ...
            std::thread new_worker(&ServerImpl::Process, this, client_socket);
            new_worker.detach();
        }
...
void ServerImpl::Process(int client_socket) {
    std::size_t arg_remains = 0;
    Protocol::Parser parser;
    std::string argument_for_command;
    std::unique_ptr<Execute::Command> command_to_execute;
    ...
    try {
        ...
        while ((readed_bytes = read(client_socket, client_buffer, sizeof(client_buffer))) > 0) {
            ...
            while (readed_bytes > 0) {
                ...
                if (command_to_execute && arg_remains > 0) {
                    ...
                    _logger->debug("Readed bytes = {}, to_read = {}, arg_remains = {}", readed_bytes, to_read, arg_remains);
                }
        ...
        if (readed_bytes == 0) {
            ...
            _logger->debug("Readed bytes!=0: {}", readed_bytes);
